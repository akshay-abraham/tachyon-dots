...
Install via Brave's own official Linux instructions (found live through Firefox on first boot — see `docs/01` step 3; their install method changes often enough that a hardcoded command here would go stale).

### Extensions
- **Bitwarden**
- **uBlockOrigin** — beyond the default lists, manually add a malware/badware filter list plus a couple of extra community lists (specific list URLs are a personal choice — add them under uBO → Filter lists → Custom).
- **Stylus** — three custom userstyles live in `fedora-kde/browser/brave/stylus/`:
  ...
  Plus a YouTube "clean" theme and a YouTube cinema-mode theme (not committed as separate files here yet — add them alongside the three above if you want them versioned too).
...
- **SponsorBlock** — configured to auto-skip most segment categories and show highlighted segments on the seek bar, so anything not auto-skipped is still visible to skip manually.
...
### Bookmarks
Handled day-to-day via Brave's Sync Chain, but it isn't fully reliable — **before any reinstall, export bookmarks manually** (see `docs/00`), and **remember to re-import them** here once Brave is freshly configured post-install.
...
Not packaged for Fedora — runs as an AppImage:

1. Download the Arduino IDE logo SVG and the AppImage itself, place both under `~/Applications/`.
...
3. Inside the Arduino IDE itself: Preferences → Additional Board Manager URLs → add the official ESP32 boards URL (get the current one from Espressif's own arduino-esp32 docs, linked from within the IDE's board manager — it's occasionally updated, so pull it live rather than from a saved copy).
4. Boards Manager → search `esp32` → install.

## Firefox

Kept as the backup browser — it's what Fedora KDE boots to by default anyway (used on first boot specifically to fetch the Brave install command, per `docs/01`). No further customization; it's the "if Brave breaks" option, not a daily driver.
...
Theme: the "modern-z" mpv skin isn't packaged — download it and drop it under `~/.config/mpv/scripts/` or `~/.config/mpv/` per that theme's own install instructions.

## Obsidian

Not in Fedora's official repos — Flatpak or AppImage. Once installed:
...
(log out/in for the group change to apply). This is also where the Arch/Hyprland experimental setup in `docs/05` actually runs, as a VM guest.
...
If it's not in your enabled repos on whatever Fedora version you're on, it's also published as a Flatpak.

## ProtonVPN

Not in Fedora's default repos — Proton publishes their own official `.rpm` repo with setup instructions on their site; add that and install the GUI client from it.

## Custom app icons

Sourced from the gdrive-synced folder at `~/Documents/Projects/Pictures/` — i.e., they arrive via the `gdrive-sync` script (`docs/02`), not this repo directly. Apply them per-app via each `.desktop` file's icon path or through Plasma's icon-picker where relevant.
...
None of these are guaranteed to be Fedora-packaged as the exact variants listed (some are proprietary/Microsoft-licensed, some are Nerd Font patches). Drop `.ttf`/`.otf` files under `~/.local/share/fonts/`, then:
